...
import face_description.dlib_api as face_recognition
...
@click.option('--cpus', default=1,
              help='number of CPU cores to use in parallel'
                   ' (can speed up processing lots of images). -1 means use all in system')
@click.option('--tolerance', default=0.6,
              help='Tolerance for face comparisons. '
                   'Default is 0.6. Lower this if you get multiple matches for the same person.')
...
def main(known_people_folder, image_to_check, cpus, tolerance, show_distance):
    ...
    if (sys.version_info < (3, 4)) and cpus != 1:
        click.echo(
            'WARNING: Multi-processing support requires Python 3.4 or greater. '
